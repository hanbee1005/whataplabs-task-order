...
주문 생성, 변경, 취소의 경우 상품의 재고 API를 호출해야 하기 때문에 외부 통신으로 인해 응답 지연이 발생할 수 있습니다.     
...
- 주문 취소 시
  - 주문 상태가 ```ORDER_COMPLETED``` 일 때 변경이 가능하며
...
- 주문 상태는 아래와 같이 5가지 상태로 구분합니다.
  ...
  + ```주문 취소 요청(ORDER_CANCEL_REQUEST)```: 주문 완료 상태에서 주문 취소를 요청했을 때 상태 (재고 원복 전)
  + ```주문 취소(ORDER_CANCELED)```: 재고 원복이 완료된 후 주문 취소 완료 상태
- 제외된 내용
  + 취소 시 취소 사유 등에 대한 내용은 제외 하였습니다.
  + 각 상태 변경에 대한 히스토리는 별도로 저장하지 않았습니다.
### 주문 상품 (Order Product)
